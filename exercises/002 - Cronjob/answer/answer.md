...
``` bash
kubectl create deployment home --image=nginx:latest --dry-run=client -o yaml > home.yaml
```
...
B:
```bash
 k create cj test --image=busybox:latest --schedule="0 0 * * *" --dry-run=client -o yaml -- "sh" "-c" "echo 'Running batch job'" > daily-batch-job.yaml
```
Then apply it:
``` bash
...
``` bash
kubectl create deployment stateless-api --image=node:18-alpine --replicas=3 --dry-run=client -o yaml > stateless-api.yaml
```
Then apply it:
```
